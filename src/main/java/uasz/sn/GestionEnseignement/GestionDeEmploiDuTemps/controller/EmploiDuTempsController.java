...
public class EmploiDuTempsController {
    ...
    public String voirEmploi(Model model, Long idMaquette, Principal principal){
        ...
        for(Seance seance : seanceService.findAll()){
            if(seanceService.findAll().size() == 1){
                seance.getEnseignant().setResponsable(true);
            }
            if(seance.getEnseignement().getMaquette() == maquette && seance.getEnseignant().isResponsable() == true){
                seances.add(seance);
            }
        }
